...
public:
	...
	int CallFunction(const char* function_name) {
      ...
      std::cerr << "Call function(" << function_name << ") failed. \"" << err.what() << ":" << lua_tostring(lua_state_, -1) << "\"" << std::endl;
		} catch (luabind::cast_failed& cf) {
			std::cerr << "Call function(" << function_name << ") failed.\"" << cf.what() << "\"" << std::endl;
		} catch (std::runtime_error& re) {
			std::cerr << "Call function(" << function_name << ") failed.\"" << re.what() << "\"" << std::endl;
		} catch (...) {
			std::cerr << "Call function(" << function_name << ") failed.\" invalid error\"." << std::endl;
		}
		return ret;
	}
	template <class A1>
